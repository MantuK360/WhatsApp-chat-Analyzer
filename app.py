...
# code from streamlit documentations
...
if uploaded_file is not None:
    ...
    df=preprocesser.preporcess(data)
    st.title("Chat Data")
    st.dataframe(df)

    # fetch unique users
    user_list=df['user'].unique().tolist()
    #user_list.remove('<Media omitted>')
    user_list.sort()
    user_list.insert(0,"overall")

    selected_user = st.sidebar.selectbox("Show Analysis users",user_list)
...
# find the busiest user in the group
       ...
       # most common words
        ...
        plt.xticks(rotation='vertical')
        # st.dataframe(most_common_df) (it show most common word in table form)
